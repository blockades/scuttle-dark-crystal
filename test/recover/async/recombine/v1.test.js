...
describe('recombine.async.recombine', context => {
  ...
  context('Returns the nickname if one is given', (assert, next) => {
    var replies = {}
    const nickname = 'Give this key to your nearest and dearest'
    share({ name, secret, quorum, nickname, recps: shardHolders }, (err, data) => {
      if (err) console.error(err)
      var rootId = data.root.key
        ...
        alice.publish(replies.alice, (err, aliceReply) => {
          ...
          bob.publish(replies.bob, (err, bobReply) => {
            ...
            recombine(rootId, (err, returnedSecret) => {
              assert.notOk(err, 'error is null')
              assert.equal(secret, returnedSecret.secret, 'returns the correct secret')
              assert.equal(nickname, returnedSecret.nickname, 'returns the correct nickname')
              next()
            })
          })
        })
